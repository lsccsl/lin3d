...
vec4 defer_render_light_spot(vec3 view_pos, vec3 view_nml)
{
	...
	vec3 view_half_vec = normalize(view_dir + light_dir);
	float spec = pow(max(dot(view_half_vec, view_nml), 0), 32);
	

	vec4 color = vec4(0, 0, 0, 1);
	float light_final = light_dot * attenuation * effect_spot;// + spec * attenuation;
	color.xyz = uni_light_clr * light_final;
	
	
//float light_final1 = light_dot * attenuation  + spec * 10 * attenuation;
//color.xyz = vec3((pow(max(dot(view_half_vec, view_nml), 0), 32) + light_dot) * attenuation / 2);
