...
sence_render_defer::sence_render_defer(sence_mgr * sence):sence_(sence)
...
void sence_render_defer::_tex_dev_sz_pool_init()
{
	/*
...
void sence_render_defer::_tex_dev_sz_pool_get(texture_base::ptr& tex)
{
	if(!tex.is_null())
...
void sence_render_defer::_tex_dev_sz_pool_release(texture_base::ptr& tex)
{
	if(tex.is_null())
		return;
...
void sence_render_defer::_init_rtt()
{
	win_device * dev = this->sence_->eng()->dev();
...
void sence_render_defer::_init_shader()
{
...
void sence_render_defer::_render_sence()
{
	win_device * dev = this->sence_->eng()->dev();
	assert(dev);
...
void sence_render_defer::_release_all_tex()
{
	{
...
void sence_render_defer::_release_reflect_tex()
{
...
void sence_render_defer::_release_refract_tex()
{
...
void sence_render_defer::_render_reflect_obj(texture_base::ptr& tex_out_put)
{
...
void sence_render_defer::_render_reflect_tex()
{
	win_device * dev = this->sence_->eng()->dev();
	assert(dev);
...
void sence_render_defer::_render_refract_tex()
{
	win_device * dev = this->sence_->eng()->dev();
	assert(dev);
...
//void sence_defer_render::_render_far_gbuffer()
//{
//	//远处的物体不需要光照
//	win_device * dev = this->sence_->eng()->dev();
//	assert(dev);
...
//
//	const sence_mgr::MAP_ROBJ& render_obj = this->sence_->map_obj_far();
//	for(sence_mgr::MAP_ROBJ::const_iterator it = render_obj.begin(); it != render_obj.end(); it ++)
//	{
//		robj_base * robj = it->second.get();
...
//void sence_defer_render::_render_far()
//{
//	this->_render_far_gbuffer();
//
//	this->_render_atmospheric();
//
//	this->ref_tex_obj_far_ = this->ref_tex_atmospheric_;
//	this->ref_tex_atmospheric_.set_null();
//}

void sence_render_defer::_render_gbuffer()
{
	win_device * dev = this->sence_->eng()->dev();
	assert(dev);
...
void sence_render_defer::_render_ssao()
{
	win_device * dev = this->sence_->eng()->dev();
	assert(dev);
...
void sence_render_defer::_render_light()
{
	win_device * dev = this->sence_->eng()->dev();
	assert(dev);
...
void sence_render_defer::_render_light_one_lighting(const light_mgr::light_info::ptr& li)
{
	win_device * dev = this->sence_->eng()->dev();
	assert(dev);
...
void sence_render_defer::_render_light_one_shadow_cast(const light_mgr::light_info::ptr& li)
{
	win_device * dev = this->sence_->eng()->dev();
	assert(dev);
...
void sence_render_defer::_render_light_one_shadow_recv(const light_mgr::light_info::ptr& li)
{
	win_device * dev = this->sence_->eng()->dev();
	assert(dev);
...
void sence_render_defer::_render_light_one_vol(const light_mgr::light_info::ptr& li)
{
	win_device * dev = this->sence_->eng()->dev();
	assert(dev);
...
void sence_render_defer::_render_light_one_mix(const light_mgr::light_info::ptr& li)
{
	win_device * dev = this->sence_->eng()->dev();
	assert(dev);
...
void sence_render_defer::_render_light_one_vol_mix(const light_mgr::light_info::ptr& li)
{
	win_device * dev = this->sence_->eng()->dev();
	assert(dev);
...
void sence_render_defer::_render_light_one(const light_mgr::light_info::ptr& li)
{
...
void sence_render_defer::_render_final(texture_base::ptr& tex_out_put)
{
	if(tex_out_put.is_null())
		return;
...
void sence_render_defer::_render_forward(texture_base::ptr& tex_out_put)
{
	if(tex_out_put.is_null())
		return;
...
void sence_render_defer::_render_atmospheric()
{
	win_device * dev = this->sence_->eng()->dev();
	assert(dev);
...
void sence_render_defer::_render_ssr(texture_base::ptr& tex_reflect_src)
{
	win_device * dev = this->sence_->eng()->dev();
	assert(dev);
...
void sence_render_defer::_debug()
{
	...
	if(this->enable_hdr_ && !this->shdr_hdr_.is_null())
	{
		l3_f32 x = 0.0f;
		for(l3_int32 i = 0; i < HDR_LEVEL; i ++){
			OBJ_ID rtt_blur = this->shdr_hdr_->rtt_blur1(i);

			this->sence_->render_show_rtt_tex(rtt_blur, 1,
				x, y,
				0.19f, 0.2f);
...
void sence_render_defer::_debug_light(l3_int32 idx)
{
