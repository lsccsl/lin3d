...
//l3_int32 shader_program::uni_unbind(const l3_int32 loc)
//{
//	MAP_GLSL_PARAM::iterator it = this->map_uniform_info_.find(loc);
//	if(this->map_uniform_info_.end() == it)
//		return -1;
//
...
//l3_int32 shader_program::uni_unbind_tex(const l3_int32 loc)
//{
	...
	const l3_f32 * pf, const l3_uint32 pf_sz)
{
	...
	buf->resize_buf_sz(sizeof(l3_f32) * pf_sz);
	l3_f32 * b = (l3_f32 *)buf->get_buf();

	memcpy(b, (void *)pf, buf->get_buf_sz());
...
l3_int32 shader_program::uni_dev_bind_tex()
{
	for(MAP_GLSL_TEX::iterator it = this->map_uniform_tex_.begin(); it != this->map_uniform_tex_.end(); it ++)
	{
		...
		l3_int32 tex_unit = this->eng_->dev()->bind_user_tex(it->second.tex_obj_, bind_shadow);
		if(tex_unit < 0)
			continue;

		glUniform1i(it->second.loc_, tex_unit);
	}
